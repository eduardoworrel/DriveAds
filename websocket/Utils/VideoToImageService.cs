...
public static class VideoToImageService
{
    ...
    public static async Task<List<string>> ConvertVideoFragmentToImagesWithRetriesAsync(
        byte[] videoData,
    ...
    )
    {
        ...
        while (attempt < MaxRetries)
        {
            ...
            try
            {
                imageBase64List = await ConvertVideoFragmentToImagesAsync(
                    videoData,
                    TimeoutMilliseconds,
                    cts
                );
    ...
    private static async Task<List<string>> ConvertVideoFragmentToImagesAsync(
        byte[] videoData,
                        ...
                        using (var memoryStream = new MemoryStream())
                        {
                            var outputTask = Task.Run(async () =>
                            {
                                ...
                                while (
                                    (
                                        bytesRead =
                                            await ffmpegProcess.StandardOutput.BaseStream.ReadAsync(
                                                buffer,
                                                0,
                                                buffer.Length
                                            )
